      ...
      grpcMethodNameList.forEach(methodName => {
        ...
        grpcAsyncMethodObject[methodName] = async (call, callback) => {
          try {
            ...
            this.logger.info(`[egg-grpc-server] use grpc method:${methodName} (${new Date().getTime() - now}ms)`);
            return await callback(null, {
              data: JSON.stringify(result),
            });
          } catch (error) {
            this.logger.error(`[egg-grpc-server] request grpc server failed , please check the error: requestMethod:${methodName}`);
            // callback(err,xx)
